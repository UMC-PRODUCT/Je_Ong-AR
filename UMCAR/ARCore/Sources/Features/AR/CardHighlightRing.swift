...
enum CardHighlightRing {
    ...
    /// 띠 중심선의 모서리 둥글기 (감싸는 대상의 한 변 대비).
    ///
    /// 인쇄된 아이콘의 모서리에 맞춘 값이다. Apple 아이콘 모서리는 한 변의
    /// 약 0.22(레퍼런스 이미지 실측 0.22~0.29, 스퀘어클이라 원호보다 길게 뻗는다).
    /// 여기에 선이 대상 밖으로 나가는 0.02와, 선이 중심선보다 안쪽에 있는
    /// 0.024(= (linePosition - 0.5) x bandWidth)를 더해 0.26이 된다.
    ///
    /// 각지게 두면 둥근 아이콘 위에 각진 액자를 씌운 꼴이 된다.
    static let cornerRadius: Float = 0.26

    /// 모서리 하나를 몇 조각으로 나눌지. 12면 90도를 7.5도씩 끊는다.
    static let cornerSteps = 12

    /// 실물 카드에서 인쇄된 아이콘이 차지하는 정사각형의 한 변 (카드 폭 대비).
    ///
    /// **테두리는 카드 외곽이 아니라 이 정사각형을 감싼다.** 카드는 90:127인데
    /// 인쇄물은 흰 배경에 아이콘 하나뿐이라(Docs/Assets/ReferenceCards/README.md의
    /// `plain` 스타일), 카드 외곽을 그리면 위아래로 빈 흰 종이를 2.4cm씩 감싸게 된다.
    ///
    /// 레퍼런스 이미지 9장을 전부 재보니 1062x1500 px 안에서 아이콘이 차지하는
    /// 칸은 935x935 px이고 가로·세로 모두 정중앙이다. 정중앙이라 위치 보정 없이
    /// 한 변만 카드 폭에서 환산하면 된다. 9cm 카드 기준 7.93cm.
    ///
    /// 네 장(cloudkit·corelocation·coreml·widgetkit)은 아이콘이 이 칸보다 살짝
    /// 홀쭉하지만, 9장의 테두리 크기를 맞추는 쪽이 부스에서 덜 어수선하다.
    ///
    /// 카드 외곽을 감싸는 예전 동작으로 되돌리려면 이 값을 1.0으로 두고
    /// CardPanelBuilder에서 깊이를 카드 세로로 되돌리면 된다.
    static let artworkSide: Float = 935.0 / 1062.0
