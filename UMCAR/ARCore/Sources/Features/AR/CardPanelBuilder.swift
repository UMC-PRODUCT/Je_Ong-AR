...
enum CardPanelBuilder {
    ...
    ///
    /// 2.2면 실물 19.8 x 27.9cm, 대략 A4다. 여기에 PanelLayout의 본문 비율을
    /// 곱하면 글자가 약 13mm로 선다.
    ///
    /// 더 키우려면 흔들림을 먼저 봐야 한다. 패널이 클수록 앵커에서 멀어져
    /// 회전 잡음이 크게 증폭된다 (PoseSmoother 참고).
    static let panelScale: Float = 2.2
