...
> ⚠️ **이 문서는 `DESIGN.md`로 대체되었다.** 아래는 전환을 처음 구상할 때 쓴 초안이고,
> 코드 대조와 실기기 스파이크를 거치며 몇 군데가 틀렸음이 확인됐다. **구현은
> `DESIGN.md`를 따를 것.** 이 문서는 어떻게 판단이 바뀌었는지의 기록으로 남긴다.
>
> 확인된 오류 (자세한 근거는 `DESIGN.md` §2):
> - **§4** "배경색을 서로 다르게" → 색이 아니라 **명암 패턴**이 달라야 한다. 인식은 휘도 기반이다.
> - **§8-②** "동시 추적 상한 4장" → 추적을 끄면 걸리지 않는다. 9장 전부 앵커가 붙는다 (실기기 확인).
> - **§3** "패널 quad (+0.05m)" → 축은 `+Z`가 아니라 **`+Y`**다.
> - **§6** "한 줄 수정으로 끝" → `GameModelMapper.swift:19`와 충돌한다.
> - **§4** 5장 기준으로 쓰였으나 카드는 **9장**이다.
> - `hasBootstrapped`는 `DataBootstrapper`가 아니라 `UMCARApp.swift:48,59`에 있다.
>
> 미결이던 §8-①(게임성)과 §8-②(카드 수·고정)는 각각 **전시물로 간다**,
> **9장·책상 고정**으로 확정됐다.
