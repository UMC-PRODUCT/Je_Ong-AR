...
```bash
CARD_STYLE=icon    python3 gen_reference_cards.py   # 기본. 아이콘이 주역
CARD_STYLE=pattern python3 gen_reference_cards.py   # 되돌릴 때
```

`icon`은 Apple 기술 아이콘을 카드 폭의 52%로 크게 놓고 그 아래 기술명·태그를 둔다.
배경 패턴은 **인식용으로 그대로 남긴다** — 아이콘만 넣으면 인식이 위험해진다.

| | 최소 셀 | 저밀도 셀 | 휘도 최소거리 |
|---|---:|---:|---:|
| `pattern` | 4 | 6/2106 | 58.0/255 |
| `icon` (채택) | 0 | 143/2106 | **55.5/255** |

**오인식 위험(휘도 최소거리)은 사실상 동등하다.** 차이는 저밀도 셀인데, 이는 아이콘과
글자판이 차지한 가운데 면이다. 가장자리는 패턴이 촘촘하다.

시도했다가 버린 것: 배경을 저가시성으로 누르면(`TONE_SCALE` 0.3) 휘도 최소거리가
**22.6/255**까지 떨어진다. 9장이 서로 구분될 근거가 배경 패턴이라 진폭을 줄이면
오인식 위험이 급등한다. 그래서 배경은 대비를 유지하고, 글자만 판을 깔아 읽히게 했다.
